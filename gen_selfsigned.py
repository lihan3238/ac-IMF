...
if __name__ == '__main__':
    ...
    # 遍历files.items()，即遍历files字典中的键值对。对于每个文件名和内容，打开文件并写入相应的内容。
    # 这样就将生成的证书和私钥保存到了文件中。
    files = {'cert.pem': cert_pem, 'key.pem': key_pem}
    for filename, content in files.items():
        # 这是一个文件操作的上下文管理器。使用open()函数打开文件，filename是文件名
        # 'wb'表示以二进制写入模式打开文件，即以二进制方式写入内容。
        # with语句可以确保在代码块执行完后，文件会被正确关闭，不需要显式调用f.close()。
        with open(filename, 'wb') as f:
            # 使用f.write()方法将content写入文件。
            f.write(content)

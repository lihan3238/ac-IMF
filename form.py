...
class PasswordForm(FlaskForm):
    # 这行代码定义了一个名为password的字段
    # PasswordField表示它用于输入密码类型的数据。字段的标签（即描述信息）被设置为"password"。
    # validators=[DataRequired()]，指定了该字段必须包含数据（不能为空），即必须输入密码。
    password = PasswordField('password', validators=[DataRequired()])

    def get_hash_password(self):
        from hashlib import sha512
        # 首先获取表单字段password的数据，即用户输入的密码。
        # 将密码编码为字节字符串，使用SHA-512哈希算法对密码进行哈希处理，并返回哈希后的结果（字节形式）。
        return sha512(self.password.data.encode()).digest()

# RegisterForm类额外添加了username和confirm_password字段，用于处理注册时的用户名和确认密码的输入。
class RegisterForm(PasswordForm):
    # 这行代码定义了一个名为username的字段，它是一个字符串输入字段（StringField）。
...
# LoginForm类额外添加了username字段，用于处理登录时的用户名的输入。
...
class FileForm(FlaskForm):
    ...
    # 这行代码定义了一个名为file的字段，它是一个文件上传字段（FileField）。FileField表示它用于处理文件上传类型的数据
